function [ output_array, Label] = CatTable2Array( input_table, IndexArray )
%--------------------------------------------------------------------------
...
%
%   version: <1.1>
%       Added the possibility to sort the table
%
...
if ~isempty(IndexArray)
    index_error=0;
    if ~isnumeric(IndexArray)||numel(IndexArray)~=size(output_array,2)
        error('Wrong sort_index')
    end
    for i = 1:size(output_array,2)
        index_error=index_error+any(IndexArray==i);
    end
    if index_error~=size(output_array,2)
        error('Wrong sort_index')
    end
    Label=Label(IndexArray);
    output_array=output_array(:,IndexArray);
end
