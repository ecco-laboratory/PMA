function [xPositions, yPositions, Label, RangeCut, FigHandles] = UnivarScatter(data, varargin)
%--------------------------------------------------------------------------
...
%                       output matrix are returned sorted alphabetically
%                       according to the categories. IndexArray allows for
%                       sorting of all columns/sets of points according to 
%                       the index. It's only used if data is a table
%
...
%                       your data.
%
%          FigHandles - An array of figures with size = [1, size(data,2)]
%                       containing the handles of each of the scatter
%                       plots, this can be specially useful for creating
%                       legends. I don't really understand what kind of
%                       output is this, since it is only numbers, but turns
%                       out it works well for the legend.
%
...
%assignation
Ind=strmatch('IndexArray',stringVars);
if ~isempty(Ind)    
    IndexArray=valueVars{Ind};
else
    IndexArray=[];
end
...
if istable(data)
    data_istable=true;
    [data,Label]=CatTable2Array(data,IndexArray);
...
%% String Variables
if ~data_istable
Ind=strmatch('Label',stringVars);
if ~isempty(Ind) && data_istable==false
    %Check that the Label is a string cell array and has a propper length
    if iscellstr(valueVars{Ind}) && length(valueVars{Ind})==size(data,2) 
        Label=valueVars{Ind};
    else
