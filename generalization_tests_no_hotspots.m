...
pos_affect_pattern_no_hotspots = apply_mask(pos_affect_pattern,all_regs,'invert');
...
pexp_pleasant_taste = apply_mask(pleasant_taste,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
pexp_not_pleasant_taste = apply_mask(not_pleasant_taste,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
...
pexp_ebdm_high_rew = apply_mask(EBDM_Cue2_hrew,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
pexp_ebdm_low_rew = apply_mask(EBDM_Cue2_lrew,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');

pexp_ebdm_high_eff = apply_mask(EBDM_Cue1_hef,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
pexp_ebdm_low_eff = apply_mask(EBDM_Cue1_lef,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');

load('C:\Users\phili\OneDrive - Emory University\Documents\DeepRL_Valence\SavedCode\outcomes.mat')
pexp_outcome_loss_incorrect = apply_mask(outcome_loss_incorrect,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
pexp_outcome_gain_correct = apply_mask(outcome_gain_correct,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
...
pexp_HC_caption = apply_mask(dat_HC_caption,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
pexp_HC_control = apply_mask(dat_control,pos_affect_pattern_no_hotspots,'pattern_expression','cosine_similarity');
...
%% plot results
load([datadir filesep 'groupvars.mat']);clear ROC_no_hotspots;
figure;
subplot(1,3,1:2)
...
ROC_no_hotspots(1) = roc_plot([pexp_ebdm_high_rew;pexp_ebdm_low_rew],[ones(26,1);zeros(26,1)],'twochoice','nofig','color',hSc(4).CData);
ROC_no_hotspots(2) = roc_plot([pexp_outcome_gain_correct(group==2);pexp_outcome_loss_incorrect(group==2)],[ones(length(pexp_outcome_gain_correct(group==2)),1); zeros(length(pexp_outcome_loss_incorrect(group==2)),1)],'twochoice','color',hSc(3).CData);
ROC_no_hotspots(3) = roc_plot([pexp_pleasant_taste;pexp_not_pleasant_taste],[ones(length(pexp_pleasant_taste),1); zeros(length(pexp_not_pleasant_taste),1)],'twochoice','color',hSc(2).CData);
ROC_no_hotspots(4) = roc_plot([pexp_HC_caption; pexp_HC_control],[ones(length(pexp_HC_caption),1);zeros(length(pexp_HC_caption),1)],'twochoice','color',hSc(1).CData);
% save ROC_no_hotspots_nhs ROC_no_hotspots
...
for nit = 1:3000
    ...
    null_ROC_no_hotspots(nit,1) = roc_plot(tv,gt,'twochoice','noplot','nooutput','noboot');
    ...
    null_ROC_no_hotspots(nit,2) = roc_plot(tv,gt,'twochoice','noplot','nooutput','noboot');
    ...
    null_ROC_no_hotspots(nit,3) = roc_plot(tv,gt,'twochoice','noplot','nooutput','noboot');
    ...
    null_ROC_no_hotspots(nit,4) = roc_plot(tv,gt,'twochoice','noplot','nooutput','noboot');
...
phat(1)=1-(1+sum([null_ROC_no_hotspots(:,1).AUC]<ROC_no_hotspots(1).AUC))/(nit+1);
phat(2)=1-(1+sum([null_ROC_no_hotspots(:,2).AUC]<ROC_no_hotspots(2).AUC))/(nit+1);
phat(3)=1-(1+sum([null_ROC_no_hotspots(:,3).AUC]<ROC_no_hotspots(3).AUC))/(nit+1);
phat(4)=1-(1+sum([null_ROC_no_hotspots(:,4).AUC]<ROC_no_hotspots(4).AUC))/(nit+1);
